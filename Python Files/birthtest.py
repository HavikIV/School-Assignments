...
#dictionary for the window titles for the paragraphs
MsgDict = {
        1:'THE ORIGINATOR',
        2:'THE PEACEMAKER',
...
def validate():   #function to valdiate the user input
        global Days
        if re.match("^[13578]|1[02]*$", Month): #checks to see if the month has 31 days
                Days = 31
        elif re.match("^[469]|[1]*$", Month): #checks to see if the month has 30 days
                Days = 30
        ...
        elif int(Day) > Days or int(Day) < 1: #checks to see if a valid day is inputted
                easygui.msgbox(MonDict[int(Month)]+ " only has " +str(Days)+ " days in it. (1-" +str(Days)+")")
                #exit() #exits the program
...
def digit(): #function to find the birth number by adding the digits until single digit is left
...
def para(): #function to show the paragraphs assoicate with the birth number
        x = digit()
...
def log():
        ...
        OF = open("Valdation Error.log", "w")
        print >> OF, FirstName, LastName, Month, Day, Year
        OF.close()
        
        #reading from log file
        IF = open("Valdation Error.log", "r")
        fieldValues = list(IF.read().split())
        IF.close()

def log2():
        #OF = open("E:/Users/Havik/Desktop/BirthTest.log", "a")
        OF = open("BirthTest.html", "w")
        HDR = """<html><head><title>BirthTest</title></head><body><pre>"""
        FTR = """</pre></body></html>"""
        OF.write(HDR + "\n" + FirstName +" "+ LastName + "\n" + "Birth Date " + Month+"/"+Day+"/"+Year +" Birth Number "+str(digit()) +"\n"+ Dict[digit()] + FTR)
        OF.close()
...
digit()
easygui.msgbox("Hello, " +FirstName+" "+LastName+", your birth number is "+str(digit()))
para()
log2()
