...
{
	if (current != NULL) //while more data to print
    {
...
void linkedListType<Type>::divideMid(linkedListType<Type> &subList)
{
	...
	if (subList.first != NULL) //if the list is not empty, make it empty
       subList.destroyList();
	
	if ((count % 2) == 0)
	{
		countDividend = count / 2;
		cout << "The countDividend is " << countDividend << endl;
	}
	else
	{
	...
	for (divCount = 0; divCount < countDividend; divCount++)
	{
		...
		{
			last = current->link;
			cout << "Printing the new last " << last << endl;
	}

	if (divCount == countDividend)
	{
